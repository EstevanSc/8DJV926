...
pub async fn start_quic_server(port: u16) -> Result<mpsc::Receiver<ShardUpdateMessage>> {
...
async fn handle_quic_message(data: Bytes, tx: mpsc::Sender<ShardUpdateMessage>) -> Result<()> {
    let boundaries =
        Boundary::decode_batch(&data).context("Failed to decode boundaries payload")?;

    tracing::debug!(
        "Received shard update from quadtree: {} boundaries",
        boundaries.len()
    );
