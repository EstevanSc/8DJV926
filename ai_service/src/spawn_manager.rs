...
fn tick_respawns(
    ...
    for b in &boundaries.0 {
        global_min_x = global_min_x.min(b.x - b.half_size);
        global_max_x = global_max_x.max(b.x + b.half_size);
        global_min_y = global_min_y.min(b.y - b.half_size);
        global_max_y = global_max_y.max(b.y + b.half_size);
    }
    ...
    let top_count = (sorted_boundaries.len() as f64 * pct).ceil() as usize;
    ...
    let mut min_x = chosen_shard.x - chosen_shard.half_size;
    let mut max_x = chosen_shard.x + chosen_shard.half_size;
    let mut min_y = chosen_shard.y - chosen_shard.half_size;
    let mut max_y = chosen_shard.y + chosen_shard.half_size;

    let padding = config.spawn_padding;
    ...
    if min_x > max_x { min_x = chosen_shard.x; max_x = chosen_shard.x; }
    if min_y > max_y { min_y = chosen_shard.y; max_y = chosen_shard.y; }
