...
pub fn on_patrol(
    ...
    tracing::debug!("Patrol triggered for entity {:?}", trigger.event().ctx().target_entity());
    ...
    tracing::debug!("Entity {:?}, at position ({:.1}, {:.1}), moving toward patrol waypoint {:?} at distance {:.1}", ctx.target_entity(), pos.x, pos.y, wp, dist);
