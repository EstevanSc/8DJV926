...
use common::topics::{
    AttributeUpdatedPayload, EntityKilledPayload, LevelUpPayload, StartingPositionPayload, Topic,
    ...
    serialize_level_up_payload, serialize_starting_position_payload, serialize_use_ability_payload,
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut client =
        match BrokerClient::connect(host.as_str(), port, SendingSystem::AbilityService).await {
            Ok(client) => {
    ...
    client
        .subscribe(Topic::PlayerStartingPosition)
        .await
        .context("Failed to subscribe to PlayerStartingPosition")?;

    client
        .subscribe(Topic::RequestCastAbility)
        .await
        .context("Failed to subscribe to RequestCastAbility")?;

    client
        .subscribe(Topic::AbilityHitEntity)
        .await
            ...
            for (topic, payload) in messages {
                match topic {
                    Topic::PlayerStartingPosition => {
                        if let Some(position_payload) =
                            deserialize_starting_position_payload(&payload)
                        {
                        ...
                        if let Some(ability_payload) = deserialize_use_ability_payload(&payload) {
                            ...
                            if let Some(entity) = entity_registry.get_mut(&entity_id) {
                                if entity.can_cast_ability(ability_payload.ability) {
                                    let (ability_type, mana_cost) = if let Some(mut_ability) =
                                        entity.find_ability_mut(ability_payload.ability)
                                    {
                                        ...
                                        tracing::warn!(
                                            "Entity {} tried to cast unregistered ability: {:?}",
                                            entity_id,
                                            ability_payload.ability
                                        );
                                        continue;
                                    };
                                    ...
                                    let cast_payload = UseAbilityPayload {
                                        entity_id,
                                        ability: ability_type,
                                        direction: ability_payload.direction,
                                    };
                                    let raw_payload = serialize_use_ability_payload(&cast_payload);
                                    if let Err(e) =
                                        client.publish_raw(cast_topic, raw_payload.as_slice()).await
                                    {
                                        tracing::error!(
                                            "Failed to publish raw ability payload: {:?}",
                                            e
                                        );
                                    }

                                    // Update entity's mana
                                    if let Some(mana_attribute) =
                                        entity.attributes.get(&AttributeType::ManaPoints)
                                    {
                                        let new_mana = mana_attribute.current_value - mana_cost;
                                        entity
                                            .update_attribute(AttributeType::ManaPoints, new_mana);
                                        ...
                                        let raw_payload = serialize_attribute_updated_payload(
                                            &attribute_updated_payload,
                                        );
                                        tracing::info!(
                                            "Mana updated for entity {}: {}",
                                            entity_id,
                                            new_mana
                                        );
                                        if let Err(e) = client
                                        ...
                                        {
                                            tracing::error!(
                                                "Failed to publish attribute updated payload: {:?}",
                                                e
                                            );
                                        }
                                    }
                    ...
                    Topic::AbilityHitEntity => {
                        if let Some(ability_payload) =
                            deserialize_ability_hit_entity_payload(&payload)
                        {
                            let hit_entity_id = ability_payload.hit_entity_id;

                            tracing::info!(
                                "AbilityHitEntity received for entity {:?}",
                                hit_entity_id
                            );
                            let mut entity_was_killed = false;
                            let caster_level = entity_registry
                                .get(&ability_payload.caster_id)
                                .map(|e| e.level)
                                .unwrap_or(1);
                            if let Some(hit_entity) = entity_registry.get_mut(&hit_entity_id) {
                                ...
                                for effect in ability.effects {
                                    if let Some(attribute) =
                                        hit_entity.attributes.get(&effect.attribute_type)
                                    {
                                        ...
                                        let new_value = attribute.current_value
                                            + (effect.modifier_value as f32 * multiplier) as i32;
                                        if let Some(updated_value) = hit_entity
                                            .update_attribute(effect.attribute_type, new_value)
                                        {
                                            // Send entity attribute update
                                            let attribute_updated_payload =
                                            ...
                                            tracing::info!(
                                                "Attribute updated for entity {}: {:?} = {}",
                                                hit_entity_id,
                                                effect.attribute_type,
                                                updated_value
                                            );
                                            ...
                                            entity_was_killed = effect.attribute_type
                                                == AttributeType::HealthPoints
                                                && updated_value <= 0;
                                            if entity_was_killed {
                                                // Send entity killed message
                                                let entity_killed_payload: EntityKilledPayload =
                                                    EntityKilledPayload {
                                                        killer_id: ability_payload.caster_id,
                                                        victim_id: ability_payload.hit_entity_id,
                                                    };
                                                let raw_payload = serialize_entity_killed_payload(
                                                    &entity_killed_payload,
                                                );
                                                if let Err(e) = client
                                                    .publish_raw(
                                                        Topic::EntityKilled(hit_entity_id),
                                                        raw_payload.as_slice(),
                                                    )
                                                    .await
                                                {
                                                    tracing::error!(
                                                        "Failed to publish entity killed payload: {:?}",
                                                        e
                                                    );
                                                }
                            ...
                            if entity_was_killed {
                                ...
                                if let Some(caster) = entity_registry.get_mut(&caster_id) {
                                    ...
                                    while caster.experience_points >= 100 {
                                        ...
                                        let raw_payload =
                                            serialize_level_up_payload(&lvl_up_payload);
                                        if let Err(e) = client
                                            .publish_raw(
                                                Topic::LevelUp(caster_id),
                                                raw_payload.as_slice(),
                                            )
                                            .await
                                        {
                                            tracing::error!(
                                                "Failed to publish entity level up payload: {:?}",
                                                e
                                            );
                                        }
                                    }
                                    ...
                                    let raw_payload =
                                        serialize_xp_earned_payload(&xp_update_payload);
                                    if let Err(e) = client
                                        .publish_raw(
                                            Topic::XPEarned(caster_id),
                                            raw_payload.as_slice(),
                                        )
                                        .await
                                    {
                                        tracing::error!(
                                            "Failed to publish entity xp update payload: {:?}",
                                            e
                                        );
                                    }
                                }
                            ...
                            tracing::error!(
                                "Failed deserialize ability hit entity payload: {:?}",
                                payload
                            );
                        }
                    }
            ...
            for (entity_id, entity) in entity_registry.iter_mut() {
                if let Some(mana_attr) = entity.attributes.get_mut(&AttributeType::ManaPoints) {
                    if mana_attr.current_value < mana_attr.max_value {
                        ...
                        let _ = client
                            .publish_raw(Topic::AttributeUpdated(*entity_id), payload.as_slice())
                            .await;
                    }
                }
            }
