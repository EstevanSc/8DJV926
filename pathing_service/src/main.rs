...
use pathfinding::{
    Graph, Node, build_portals_from_nodes, has_line_of_sight, run_a_star, run_funnel,
};
...
use common::topics::{
    PathRequestPayload, PathResponsePayload, Topic, deserialize_path_request_payload,
    serialize_path_response_payload,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    run_main_loop(broker_client).await
}

async fn run_main_loop(broker_client: Option<QuicClient>) -> anyhow::Result<()> {
...
async fn handle_quic_message(data: &[u8], broker: &QuicClient, graph: &Graph) {
...
async fn handle_path_request(request: &PathRequestPayload, broker: &QuicClient, graph: &Graph) {
    /*tracing::info!(
        "Received path request for entity {} from {:?} to {:?}",
        request.entity_id,
        request.start,
        request.end
    );*/
    ...
    if shifted_start_x < 0.0
        || shifted_start_y < 0.0
        || shifted_end_x < 0.0
        || shifted_end_y < 0.0
        || shifted_start_x >= map_world_width
        || shifted_start_y >= map_world_height
        || shifted_end_x >= map_world_width
        || shifted_end_y >= map_world_height
    {
    ...
    if let Err(e) = broker.publish(topic, &response_bytes).await {
        tracing::error!(
            "Failed to publish path response for entity {}: {}",
            request.entity_id,
            e
        );
    }
