...
use common::topics::{
    PositionPayload, QuadtreeBoundariesUpdatePayload, ReleaseOwnershipPayload,
    StartingPositionPayload, Topic, deserialize_position_payload,
    deserialize_shard_created_payload, deserialize_starting_position_payload,
    serialize_position_payload, serialize_quadtree_boundaries_update_payload,
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        "Quadtree starting with config: world_size={}, max_capacity={}, max_depth={}, nearby_margin={}, area_of_interest_radius={}",
        config.world_size,
        config.max_capacity,
        config.max_depth,
        config.nearby_margin,
        config.area_of_interest_radius
    );

    // Connect to orchestrator and broker via separate QUIC links.
    let orchestrator_client =
        match QuicClient::connect_orchestrator(&config.orchestrator_host, config.orchestrator_port)
            .await
        {
            Ok(client) => {
                tracing::info!(
            ...
            Err(e) => {
                tracing::error!(
                    "Failed to connect to orchestrator: {}. Running without QUIC updates.",
                    e
                );
                None
            }
    ...
    let broker_client =
        match QuicClient::connect_broker(&config.broker_host, config.broker_port).await {
            Ok(client) => {
                tracing::info!(
...
async fn run_main_loop(
    ...
    if let Some(client) = broker_client.as_ref() {
        ...
        client
            .subscribe(client_id, Topic::PlayerStartingPosition)
            .await?;
        tracing::info!(
    ...
    if let Some(client) = orchestrator_client.as_ref() {
        ...
        tracing::warn!(
            "No connection to orchestrator, skipping initial shard configuration update"
        );
    }
    ...
    loop {
        ...
        if let Some(client) = broker_client.as_mut() {
            poll_quic_events(
                ...
                &entity_map,
                &entity_owners,
                &mut flagged_for_rebuild,
                &mut should_handle_pending_destructions,
            )
            .await?;
        }

        if let Some(client) = broker_client.as_ref() {
            process_pending_players(
                &pending_players,
                client,
                &shard_map,
                &entity_map,
        ...
        if flagged_for_rebuild || should_handle_pending_destructions {
            ...
            if new_shard_set != old_shard_set || should_handle_pending_destructions {
                ...
                tracing::info!(
                    "Shard set changed after rebuild, sending updated configuration to orchestrator..."
                );
                if let Some(client) = orchestrator_client.as_ref() {
                    ...
                    tracing::warn!(
                        "No connection to orchestrator, skipping shard configuration update"
                    );
                }
                let new_boundaries: Vec<Boundary> =
                    shard_set.read().unwrap().iter().copied().collect();
                //broadcast the new shard boundaries to all connected clients
                let payload = serialize_quadtree_boundaries_update_payload(
                    &QuadtreeBoundariesUpdatePayload {
                ...
                if let Some(broker) = broker_client.as_ref() {
                    broker
                        .publish(Topic::QuadtreeBoundariesUpdate, &payload)
                        .await?;
                }
        ...
        match event {
            GameNetworkEvent::Message {
                ...
                handle_quic_message(
                    ...
                    flagged_for_rebuild,
                    broker,
                    should_handle_pending_destructions,
                )
                .await;
            }
            _ => {}
        ...
        // 5. Sync shard_map without wiping out existing UUID connections
        {
            ...
            for leaf in final_leaves {
                ...
                for pending_shard in pending_shard_to_destroy.iter() {
                    if pending_shard.1.x == leaf.x
                        && pending_shard.1.y == leaf.y
                        && pending_shard.1.half_size == leaf.half_size
                    {
                        tracing::info!(
                            "Retaining old shard {:?} in shard_map during rebuild to avoid pending destruction",
                            leaf
                        );
...
async fn send_server_configuration_update(
    client: &QuicClient,
...
async fn handle_quic_message(
    ...
    match message {
        BrokerMessage::Broadcast { topic, payload } | BrokerMessage::Publish { topic, payload } => {
            match Topic::from_bytes(topic) {
                Topic::PlayerStartingPosition => {
                    handle_player_starting_position_topic(&payload, pending_players);
                }
                Topic::ShardCreated => {
                    handle_shard_created_topic(
                        &payload,
                        shard_map,
                        flagged_for_rebuild,
                        entity_map,
                ...
                Topic::EntityPositionUpdate(uuid) => {
                    handle_entity_position_update_topic(
                        uuid,
                        &payload,
                        shard_map,
                        entity_map,
                ...
                Topic::Disconnect(uuid) => {
                    ...
                    if let Err(e) = broker
                        .unsubscribe(uuid, Topic::EntityPositionUpdate(uuid))
                        .await
                    {
                        tracing::error!(
                            "Failed to unsubscribe from position updates for entity {:?}: {}",
                            uuid,
                            e
                        );
                    } else {
                        tracing::info!(
                            "Unsubscribed from position updates for entity {:?} due to disconnect",
                            uuid
                        );
                    }
                    if let Err(e) = broker
                        .unsubscribe(uuid, Topic::AttributeUpdated(uuid))
                        .await
                    {
                        tracing::error!(
                            "Failed to unsubscribe from attribute updates for entity {:?}: {}",
                            uuid,
                            e
                        );
                    } else {
                        tracing::info!(
                            "Unsubscribed from attribute updates for entity {:?} due to disconnect",
                            uuid
                        );
                    }
...
async fn handle_shard_created_topic(
    ...
    *should_handle_pending_destructions = true;
    ...
    tracing::info!(
        ...
        parsed.shard_connection_id,
        parsed.boundary.x,
        parsed.boundary.y,
        parsed.boundary.half_size
    );
    ...
    for (entity_id, data) in entity_map.read().unwrap().iter() {
        if parsed
            .boundary
            .contains(&data.position[0], &data.position[1])
        {
            tracing::info!(
                "Subscribing new shard shard_uuid={} to position updates for entity_id={} due to shard creation",
                parsed.shard_connection_id,
                entity_id
            );
            let topic = Topic::EntityPositionUpdate(*entity_id);
            broker
                .subscribe(parsed.shard_connection_id, topic)
                .await
                .unwrap_or_else(|e| {
                    tracing::error!(
                        "Failed to subscribe new shard to entity position updates: {}",
                        e
                    )
                });
            ...
            broker
                .publish(Topic::EntityPositionUpdate(*entity_id), &payload)
                .await
                .unwrap_or_else(|e| {
                    tracing::error!("Failed to publish entity position to new shard: {}", e)
                });
        }
    }
}
...
fn handle_player_starting_position_payload(
    ...
    match pending_players.try_lock() {
        ...
        Err(_) => {
            tracing::warn!(
                "Pending players queue contended, dropping spawn request for player_id={}",
                payload.connection_id
            );
        }
    }
...
fn find_shard_for_position(
    shard_map: &SharedShardMap,
    x: f64,
    y: f64,
) -> Option<(Boundary, Option<uuid::Uuid>)> {
...
async fn spawn_player_on_shard(
    ...
    tracing::info!(
        "Spawning player player_id={} on shard shard_uuid={}",
        player_id,
        shard_uuid
    );
    ...
    broker
        .subscribe(shard_uuid, Topic::PlayerStartingPositionInShard(player_id))
        .await?;
    broker
        .subscribe(shard_uuid, Topic::Input(player_id))
        .await?;
    broker
        .subscribe(shard_uuid, Topic::Disconnect(player_id))
        .await?;
    broker
        .subscribe(shard_uuid, Topic::CastAbility(player_id))
        .await?;
    broker
        .subscribe(shard_uuid, Topic::EntityKilled(player_id))
        .await?;

    //subscribe the client to the player's position updates so it can track its own position for interpolation
    broker
        .subscribe(player_id, Topic::EntityPositionUpdate(player_id))
        .await?;
    broker
        .subscribe(player_id, Topic::CastAbility(player_id))
        .await?;
    broker
        .subscribe(player_id, Topic::AttributeUpdated(player_id))
        .await?;

    //send the initial position update so the client and quadtree have a baseline position for the player
    let payload = serialize_position_payload(&PositionPayload { position });

    broker
        .publish(Topic::EntityPositionUpdate(player_id), &payload)
        .await?;
    ...
    broker
        .subscribe(
            ...
            Topic::EntityPositionUpdate(player_id),
        )
        .await?;
    broker
        .subscribe(broker.connection_id(), Topic::Disconnect(player_id))
        .await?;

    let boundary = find_shard_for_position(shard_map, position[0], position[1])
        .map(|(b, _)| b)
        .unwrap_or_else(|| Boundary {
            x: 0.0,
            y: 0.0,
            half_size: 100.0,
        });
    ...
    broker
        .publish(
            Topic::PlayerStartingPositionInShard(player_id),
            &payload_bytes,
        )
        .await?;
...
async fn process_pending_players(
    ...
    for (shard_uuid, player_id, position) in ready {
        if let Err(e) = spawn_player_on_shard(
            broker,
            shard_uuid,
            player_id,
            position,
        ...
        {
            tracing::error!(
                "Failed to spawn player player_id={} on shard shard_uuid={}: {}",
                player_id,
                shard_uuid,
                e
            );
            ...
            tracing::info!(
                "Successfully processed pending spawn for player_id={}",
                player_id
            );
        }
    }
...
async fn handle_entity_position_update_topic(
    ...
    if !is_known_entity {
        tracing::warn!(
            "Received position update for unknown entity_id={}, treating as new spawn at ({}, {})",
            connection_id,
            parsed.position[0],
            parsed.position[1]
        );
    ...
    let (entities_in_interest, ghosted_boundaries, old_shard, new_shard) = {
        ...
        let old_shard =
            old_position.and_then(|pos| find_shard_for_position(shard_map, pos[0], pos[1]));
    ...
    check_for_shard_ghosting(broker, connection_id, entity_map, shard_map, entity_owners).await;
...
async fn apply_area_of_interest(broker: &QuicClient, entity_map: &SharedEntityMap) {
...

    {
        ...
        for (entity_id, data) in entity_map_reader.iter() {
            ...
            for (other_id, other_data) in entity_map_reader.iter() {
                ...
                let radius_squared = Config::from_env().area_of_interest_radius
                    * Config::from_env().area_of_interest_radius;
            ...
            let new_interests: HashSet<uuid::Uuid> = nearby_entities
                .difference(&data.entities_in_interest)
                .cloned()
                .collect();
            let no_longer_in_interest: HashSet<uuid::Uuid> = data
                .entities_in_interest
                .difference(&nearby_entities)
                .cloned()
                .collect();
    ...
    for (entity_id, _, new_interests, no_longer_in_interest) in pending_updates {
        for new_id in new_interests {
            ...
            if let Err(e) = broker
                .subscribe(entity_id, Topic::EntityPositionUpdate(new_id))
                .await
            {
                tracing::error!(
                    "Failed to subscribe to position updates for entity {:?}: {}",
                    new_id,
                    e
                );
            } else {
                tracing::info!(
                    "Subscribed to position updates for entity {:?} as it entered the area of interest of entity {:?}",
                    new_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker.subscribe(entity_id, Topic::Disconnect(new_id)).await {
                tracing::error!(
                    "Failed to subscribe to disconnect updates for entity {:?}: {}",
                    new_id,
                    e
                );
            } else {
                tracing::info!(
                    "Subscribed to disconnect updates for entity {:?} as it entered the area of interest of entity {:?}",
                    new_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker
                .subscribe(entity_id, Topic::CastAbility(new_id))
                .await
            {
                tracing::error!(
                    "Failed to subscribe to cast ability updates for entity {:?}: {}",
                    new_id,
                    e
                );
            } else {
                tracing::info!(
                    "Subscribed to cast ability updates for entity {:?} as it entered the area of interest of entity {:?}",
                    new_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker
                .subscribe(entity_id, Topic::AttributeUpdated(new_id))
                .await
            {
                tracing::error!(
                    "Failed to subscribe to attribute updates for entity {:?}: {}",
                    new_id,
                    e
                );
            } else {
                tracing::info!(
                    "Subscribed to attribute updates for entity {:?} as it entered the area of interest of entity {:?}",
                    new_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker.subscribe(entity_id, Topic::LevelUp(new_id)).await {
                tracing::error!(
                    "Failed to subscribe to level up updates for entity {:?}: {}",
                    new_id,
                    e
                );
            } else {
                tracing::info!(
                    "Subscribed to level up updates for entity {:?} as it entered the area of interest of entity {:?}",
                    new_id,
                    entity_id
                );
            }
        ...
        for old_id in no_longer_in_interest {
            ...
            if let Err(e) = broker
                .unsubscribe(entity_id, Topic::EntityPositionUpdate(old_id))
                .await
            {
                tracing::error!(
                    "Failed to unsubscribe from position updates for entity {:?}: {}",
                ...
                tracing::info!(
                    "Unsubscribed from position updates for entity {:?} as it left the area of interest of entity {:?}",
                    old_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker
                .unsubscribe(entity_id, Topic::Disconnect(old_id))
                .await
            {
                tracing::error!(
                    "Failed to unsubscribe from disconnect updates for entity {:?}: {}",
                    old_id,
                    e
                );
            } else {
                tracing::info!(
                    "Unsubscribed from disconnect updates for entity {:?} as it left the area of interest of entity {:?}",
                    old_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker
                .unsubscribe(entity_id, Topic::CastAbility(old_id))
                .await
            {
                tracing::error!(
                    "Failed to unsubscribe from cast ability updates for entity {:?}: {}",
                    old_id,
                    e
                );
            } else {
                tracing::info!(
                    "Unsubscribed from cast ability updates for entity {:?} as it left the area of interest of entity {:?}",
                    old_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker
                .unsubscribe(entity_id, Topic::AttributeUpdated(old_id))
                .await
            {
                tracing::error!(
                    "Failed to unsubscribe from attribute updates for entity {:?}: {}",
                ...
                tracing::info!(
                    "Unsubscribed from attribute updates for entity {:?} as it left the area of interest of entity {:?}",
                    old_id,
                    entity_id
                );
            }
            ...
            if let Err(e) = broker.unsubscribe(entity_id, Topic::LevelUp(old_id)).await {
                tracing::error!(
                    "Failed to unsubscribe from level up updates for entity {:?}: {}",
                    old_id,
                    e
                );
            } else {
                tracing::info!(
                    "Unsubscribed from level up updates for entity {:?} as it left the area of interest of entity {:?}",
                    old_id,
                    entity_id
                );
            }
...
async fn check_for_handoff(
    ...
    if !is_within_margin(
        ...
        if let Some(new_shard_uuid) = shard_map
            .read()
            .unwrap()
            .get(&new_shard)
            .and_then(|uuid| *uuid)
        {
            ...
            if let Some(old_shard_uuid) = shard_map
                .read()
                .unwrap()
                .get(&old_shard)
                .and_then(|uuid| *uuid)
            {
                ...
                broker
                    .publish(Topic::ReleaseOwnership(old_shard_uuid), &payload)
                    .await
                    .ok();
            } else {
                // Release ownership will send the claim otherwise to ensure one authority at a time
                broker
                    .publish(Topic::ClaimOwnership(new_shard_uuid), entity_id.as_bytes())
                    .await
                    .ok();
            }
            ...
            entity_owners
                .write()
                .unwrap()
                .insert(entity_id, new_shard_uuid);
...
async fn check_for_shard_ghosting(
        ...
        map.iter()
            .filter_map(|(boundary, maybe_uuid)| {
                if *boundary == current_boundary
                    || !is_within_margin(boundary, position[0], position[1], nearby_margin)
                {
                    return None;
                }
    ...
    for (boundary, shard_uuid) in &desired_ghosts {
        ...
        if let Err(e) = broker
            .subscribe(*shard_uuid, Topic::EntityPositionUpdate(entity_id))
            .await
        {
            tracing::error!(
                "Failed to subscribe to shard {:?} to position updates for entity {:?}: {}",
                shard_uuid,
                entity_id,
                e
            );
            ...
            tracing::info!(
                "Subscribed shard {:?} to position updates for entity {:?} as it is within nearby margin of the shard boundary",
                shard_uuid,
                entity_id
            );
        }
    ...
    for (_, shard_uuid) in &stale_ghosts {
        if let Err(e) = broker
            .unsubscribe(*shard_uuid, Topic::EntityPositionUpdate(entity_id))
            .await
        {
            tracing::error!(
                "Failed to unsubscribe shard {:?} from position updates for entity {:?}: {}",
                shard_uuid,
                entity_id,
                e
            );
            ...
            tracing::info!(
                "Unsubscribed shard {:?} from position updates for entity {:?} after leaving nearby margin",
                shard_uuid,
                entity_id
            );
        }
...
async fn retain_old_shards_during_rebuild(
    ...
    for new_shard in new_shard_set {
        if let Some(_) = shard_map.get(new_shard).and_then(|id| *id) {
            ...
            let parent_shard = old_shard_set.iter().find(|old| {
                old.contains(&new_shard.x, &new_shard.y) && old.half_size > new_shard.half_size
            });
            if let Some(parent) = parent_shard {
                ...
                tracing::info!(
                    "keeping parent shard for new shard ({}, {}, {}) during rebuild: parent boundary=({}, {}, {})",
                    new_shard.x,
                    new_shard.y,
                    new_shard.half_size,
                    parent.x,
                ...
                if !parent_uuid.is_some() {
                    tracing::warn!(
                        "Parent shard boundary=({}, {}, {}) for new shard ({}, {}, {}) has no registered shard UUID, not marking for destruction (maybe already done)",
                        parent.x,
                        parent.y,
                        parent.half_size,
                        new_shard.x,
                    ...
                    tracing::info!(
                        "Parent shard boundary=({}, {}, {}) for new shard ({}, {}, {}) has registered shard UUID {:?}, retaining during rebuild",
                        parent.x,
                        parent.y,
                        parent.half_size,
                        new_shard.x,
                ...
                let childrens = old_shard_set
                    .iter()
                    .filter(|old| new_shard.contains(&old.x, &old.y))
                    .copied()
                    .collect::<Vec<_>>();
                tracing::info!(
                    "keeping children shards for new shard ({}, {}, {}) during rebuild: {} children found",
                    new_shard.x,
                    new_shard.y,
                    new_shard.half_size,
                    childrens.len()
                ...
                for child in childrens {
                    if child.x == new_shard.x
                        && child.y == new_shard.y
                        && child.half_size == new_shard.half_size
                    {
                    ...
                    if !child_uuid.is_some() {
                        tracing::warn!(
                            "Child shard boundary=({}, {}, {}) for new shard ({}, {}, {}) has no registered shard UUID, not marking for destruction (may be already done)",
                            child.x,
                            child.y,
                            child.half_size,
                            new_shard.x,
                        ...
                        tracing::info!(
                            "Child shard boundary=({}, {}, {}) for new shard ({}, {}, {}) has registered shard UUID {:?}, retaining during rebuild",
                            child.x,
                            child.y,
                            child.half_size,
                            new_shard.x,
...
async fn handle_shard_destruction(
    ...
    let entities_to_process = {
        ...
        map.iter()
            .filter(|(_, data)| {
                destroyed_boundary.contains(&data.position[0], &data.position[1])
                    || is_within_margin(
                        &destroyed_boundary,
                        data.position[0],
                        data.position[1],
    ...
    tracing::info!(
        "Processing shard destruction for shard_uuid={} with boundary=({}, {}, {}): {} entities found within or near the destroyed shard",
        destroyed_shard_uuid,
        destroyed_boundary.x,
        destroyed_boundary.y,
        destroyed_boundary.half_size,
        entities_to_process.len()
    );
    for (entity_id, position) in entities_to_process {
        if let Some((_, Some(new_shard_uuid))) =
            find_shard_for_position(shard_map, position[0], position[1])
        {
            // Prevent attempting handoff to itself
            if new_shard_uuid == destroyed_shard_uuid {
                tracing::warn!(
                    "shard_for_position returned the destroyed shard itself for entity_id={}, skipping handoff",
                    entity_id
                );
                continue;
            }
            ...
            if is_owned {
                ...
                let _ = broker
                    .publish(Topic::ReleaseOwnership(destroyed_shard_uuid), &payload)
                    .await;
                entity_owners
                    .write()
                    .unwrap()
                ...
                let _ = broker
                    .unsubscribe(new_shard_uuid, Topic::EntityPositionUpdate(entity_id))
                    .await;
            } else {
                let _ = broker
                    .subscribe(new_shard_uuid, Topic::EntityPositionUpdate(entity_id))
                    .await;
            }
        }
    }
