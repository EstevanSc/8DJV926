...
use crate::authority::{
    AuthorityEnvelope, HandoffAccept, HandoffReject, HandoffRequest,
};
...
fn setup_pending_handoff(mut commands: Commands) {
    let entity = commands
        ...
        .id();
    begin_handoff(&mut commands, entity, 7, sample_request(100), 12);
...
fn spawn_ghost(mut commands: Commands) {
    let entity = crate::authority::spawn_ghost_entity(&mut commands, 55, 13, Vec2::new(1.0, 2.0));
    commands.insert_resource(TestEntity(entity));
}
...
/// Verifies a handoff accept restores Owned state.
#[test]
fn handoff_accept_finalizes_owned() {
    let mut app = App::new();
    app.add_plugins(MinimalPlugins);
